...
from resume_parser import (
    ...
    extract_keywords_from_jd, is_likely_jd_by_filename, jd_likelihood_score
)
...
if "_cookie_checked" not in st.session_state:
    ...
    if COOKIES_OK:
        try:
            saved_email = _cookies.get("joy_email")
            saved_pass  = _cookies.get("joy_pass")
            if saved_email and saved_pass and "@" in saved_email:
                st.session_state.authenticated = True
                st.session_state.username      = saved_email
                st.session_state.name          = get_first_name(saved_email)
                st.session_state.smtp_email    = saved_email
                st.session_state.smtp_password = saved_pass
                st.session_state.sender_name   = get_first_name(saved_email)
        except:
            pass

def do_login(email, app_pass):
    ukey = email.strip().lower()
    st.session_state.authenticated = True
    st.session_state.username      = ukey
    st.session_state.name          = get_first_name(ukey)
    st.session_state.smtp_email    = ukey
    st.session_state.smtp_password = app_pass.strip()
    st.session_state.sender_name   = get_first_name(ukey)
    st.session_state.chat          = []
    st.session_state.page          = "main"
    log_login(ukey)
    if COOKIES_OK:
        try:
            _cookies.set("joy_email", ukey,            max_age=60*60*24*365)
            _cookies.set("joy_pass",  app_pass.strip(), max_age=60*60*24*365)
        except:
            pass
...
if not st.session_state.get("authenticated", False):
    ...
    with st.form("login_form"):
        email    = st.text_input("Gmail / Work Email", placeholder="you@company.com")
        app_pass = st.text_input("App Password", type="password", placeholder="16-character app password")
        st.caption("Google Account → Security → 2-Step Verification → App Passwords → Mail → Generate")
        ok = st.form_submit_button("Sign in", use_container_width=True)

    if ok:
        if email.strip() and "@" in email and len(app_pass.strip()) >= 16:
            do_login(email, app_pass)
            st.rerun()
        else:
            st.error("Enter a valid email and 16-character App Password.")
    st.stop()

# ─────────────────────────────────────────────────────────────────
# SESSION STATE DEFAULTS (post-login)
# ─────────────────────────────────────────────────────────────────
for k, v in {
    "chat": [], "results_df": None, "role_detected": "",
    "industry_detected": "", "generated_jd": "", "jd_role": "",
    "uploads": [], "show_outreach": False, "page": "main",
}.items():
    if k not in st.session_state:
        st.session_state[k] = v

if not st.session_state.get("sender_name"):
    st.session_state.sender_name = st.session_state.get("name", "")

def do_logout():
    if COOKIES_OK:
        try: _cookies.remove("joy_email"); _cookies.remove("joy_pass")
        except: pass
