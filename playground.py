...
# ─────────────────────────────────────────────────────────────────
# SEMANTIC EMBEDDING + FUZZY + VECTOR STORE
# ─────────────────────────────────────────────────────────────────
    ...
    if files:
        ...
        if not jd_text:
            ...
            if best[2] >= 40:
                jd_text = best[1]
                jd_candidate = best[0]
                joy(f"✅ **JD auto-detected by content analysis**: **{jd_candidate}**")
        ...
        with st.spinner(f"Screening {len(resume_texts)} resumes with semantic embeddings..."):
            ...
            for fname, text in resume_texts:
                ...
                vector_store.add(fname, resume_emb, {"name": name, "score": semantic_score})
