...
if not st.session_state.sender_password:
    with st.expander("Gmail App Password required for sending emails", expanded=True):
        st.caption(
            "Google blocks regular passwords for SMTP. Generate a 16-character "
...
with st.sidebar:
    ...
    st.session_state.sender_name = st.text_input(
        "Sender name", value=st.session_state.sender_name, placeholder="Your name"
    )
    st.session_state.company_name = st.text_input(
        "Company", value=st.session_state.company_name, placeholder=DEFAULT_COMPANY
    )
    ...
    # Stable identifier for Supabase history / JD library — do not change this key mid-session
...
with screen_tab:
    ...
    jd_upload = st.file_uploader(
        "Upload JD", type=["pdf", "docx", "txt"], key=f"jd_upload_{st.session_state.upload_session}"
    )
    ...
    with st.expander("Optional screening controls (Persona)", expanded=False):
        ...
        if picked == NEW_CLIENT_LABEL:
            client_company_input = st.text_input(
                "Client name", placeholder="e.g. Atomgrid", key="client_company_input"
            )
        else:
        ...
        extra_keywords = st.text_input(
            "Must-have keywords", placeholder="HPLC, distributor management, SAP", key="extra_keywords"
        )
        ...
        if client_company_input.strip():
            ...
            profile["preferred_colleges"] = st.text_area(
                ...
                value=profile.get("preferred_colleges", "") or "",
            ...
            profile["culture_notes"] = st.text_area(
                ...
                value=profile.get("culture_notes", "") or "",
            ...
            if st.button("Save Persona", type="secondary", use_container_width=True):
                ...
                st.session_state["_persona_save_status"] = "success" if saved_ok else "error"
                if saved_ok:
                    st.session_state["_persona_profile"] = profile
                    clean_name = client_company_input.strip()
                    if clean_name and clean_name not in st.session_state.known_clients:
                        st.session_state.known_clients.insert(0, clean_name)
                st.rerun()
    ...
    if run_clicked:
        if not uploads:
            ...
            st.session_state.last_keywords = extra_keywords
            ...
            try:
                ...
                st.warning(f"History save failed: {hist_err}")

            st.success(f"Screened {len(results)} resumes for {detected_role}.")
...
with email_tab:
    ...
    if st.session_state.results_df.empty:
        ...
        editable = editable.drop(
            columns=["Reason", "Duplicate", "Profile Key", "Keyword Score", "Semantic Score"],
            errors="ignore",
        )
        editable = order_columns_first(
            editable,
            ["Rank", "Send", "Name", "Email", "Phone", "Experience", "Education", "Final Score", "Feedback", "Verdict"],
        )
        ...
        edited = st.data_editor(
            ...
            num_rows="fixed",
            disabled=[
                "Rank", "Phone", "Experience", "Education", "Final Score", "Verdict",
                "Industry Match", "Candidate Industry", "Matched Keywords", "Missing Keywords",
                "Skills", "Source File", "AI Used", "Feedback",
            ],
            column_config={
        ...
        missing_email = st.session_state.selected_candidates[
            st.session_state.selected_candidates["Email"].astype(str).str.contains("", na=False) == False
        ] if False else pd.DataFrame()
        # (kept simple missing-email check below)
        missing_email = st.session_state.selected_candidates[
            ~st.session_state.selected_candidates["Email"].astype(str).str.contains("@", na=False)
        ]

        st.session_state.questions_text = st.text_area(
            "Questions to collect", value=st.session_state.questions_text, height=180
        )
...
with history_tab:
    ...
    if hist.empty:
        ...
        c2.metric(
            "Strong Fit",
            int((hist["Verdict"] == "Strong Fit").sum()) if "Verdict" in hist.columns else 0,
        )
            ...
            if selected_role != "all":
                if st.button(
                    ...
                    def delete_role_dialog():
                        st.warning(f"This will permanently delete **all screenings** for **{selected_role}**.")
                        ...
                        with col1:
                            if st.button("Cancel", use_container_width=True, key="cancel_del_role"):
                                st.rerun()
                        with col2:
                            if st.button("Yes, delete role", type="primary", use_container_width=True, key="confirm_del_role"):
        ...
        with delete_col2:
            if st.button(
                ...
                def delete_all_dialog():
                    st.error("**Warning:** This permanently deletes **all** screening history.")
                    st.write("This action cannot be undone.")
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("Cancel", use_container_width=True, key="cancel_del_all"):
                            st.rerun()
                    with col2:
                        if st.button("Yes, Delete Everything", type="primary", use_container_width=True, key="confirm_del_all"):
        ...
        history_editable = history_editable.drop(
            columns=["Reason", "JD", "Duplicate", "Keyword Score", "Semantic Score", "id", "user_key", "created_at"],
            errors="ignore",
        )
        ...
        preferred_order = ["Name", "Email", "Phone", "Experience", "Education", "Verdict", "Feedback"]
        ...
        if "Phone" in history_editable.columns:
            history_editable["Phone"] = (
                history_editable["Phone"].fillna("").astype(str).str.replace(r"\.0$", "", regex=True)
            )

        if "Feedback" not in history_editable.columns:
            history_editable["Feedback"] = "Pending"
        history_editable["Feedback"] = history_editable["Feedback"].fillna("Pending").replace("", "Pending")
        ...
        history_edited = st.data_editor(
            ...
            column_config={
                ...
                "Feedback": st.column_config.SelectboxColumn(
                    "Feedback",
                    options=["Pending", "Interviewed", "Shortlisted", "Hired", "Rejected", "Do Not Consider"],
                ),
            },
        )
        ...
        if not st.session_state.selected_history.empty:
            ...
            history_questions = st.text_area(
                "Questions to collect", value=st.session_state.questions_text, height=180, key="history_questions"
            )
            history_note = st.text_area(
                "Extra note", placeholder="Optional context for candidates", height=100, key="history_note"
            )
...
with jd_tab:
    ...
    with st.expander("Save current JD", expanded=False):
        ...
        jd_save_role = st.text_input("Role", key="jd_save_role")
        jd_save_text = st.text_area("JD Text", height=220, key="jd_save_text")
        jd_save_tags = st.text_input(
            "Tags", placeholder="e.g. sales, agrochemical, west india", key="jd_save_tags"
        )
    ...
    if not jd_df.empty:
        ...
        jd_options = [
            f"{row['Role']} · {str(row.get('Saved At', ''))[:19]}" for _, row in jd_df.iterrows()
        ]
        ...
        st.caption(f"Tags: {picked_row.get('Tags', '') or '—'}")

        preview_jd = str(picked_row.get("JD Text", "") or "")
        st.text_area("Saved JD preview", value=preview_jd, height=260, disabled=True)
        ...
        with action_col2:
            if st.button("Delete JD", use_container_width=True, type="secondary"):
                ...
                def delete_jd_dialog():
                    st.warning(f"This will permanently delete the saved JD for **{role_label}**.")
                    st.write("This action cannot be undone.")
                    col1, col2 = st.columns(2)
                    with col1:
