...
from .constants import DEFAULT_QUESTIONS, DEFAULT_COMPANY
...
def get_learning_adjustments(user_key: str, client_company: str):
    ...
    if "Profile Key" in hist.columns:
        for _, row in hist.iterrows():
            ...
            fb = str(row.get("Feedback", "Pending")).strip()
...
def apply_candidate_memory(row: pd.Series, candidate_memory: dict) -> tuple:
    pk = row.get("Profile Key", "")
    ...
    if feedback == "Good Hire":
        return 12.0, f"Previously a Good Hire ({memory['role']})."
    if feedback == "Bad Hire":
        return -15.0, f"Previously a Bad Hire ({memory['role']})."
    if feedback == "Not Selected":
        return -4.0, f"Previously Not Selected ({memory['role']})."
    return 0.0, ""


def _required_education_from_jd(jd_requirements: dict) -> tuple:
    """
    parse_required_education_level() takes the education requirement
    STRING (e.g. "B.Tech in Mechanical Engineering"), not the raw JD text,
    and returns just an int level — not a tuple. The requirement string
    itself comes from extract_jd_requirements_ai()'s 'required_education'
    field. This was called wrong last turn (passed jd_text, expected a
    tuple back) — fixed here.
    """
...
def run_screening(
    ...
    role = role_input.strip() if role_input and role_input.strip() else detect_role_title(
        jd_text, role_input, api_key, model
    )
    ...
    # extract_keywords() merges configured extras + AI-extracted core
    # skills/tools from jd_requirements + heuristic frequency keywords,
    # then runs them through clean_keywords() to strip JD boilerplate.
    # This is the real keyword source the "Missing Keywords" column needs —
    # last turn's extract_keywords_from_jd() alias silently ignored jd_req.
    ...
    for i, file in enumerate(uploads):
        try:
            ...
            if total_adjustment:
                row["Final Score"] = max(0.0, min(100.0, round(row["Final Score"] + total_adjustment, 1)))
