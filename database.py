...
def save_chat_session(user: str, messages: list):
    """
    Save a completed chat session with a preview label.
    Keeps last 20 sessions.
    """
    if not messages:
        return
    path = f"chat_sessions_{user}.json"
    # Generate preview from first user message
    preview = next((m["content"][:40] for m in messages if m.get("role") == "user"), "Chat")
    session = {
        "preview": preview,
        "messages": messages,
...
def load_chat_sessions(user: str) -> list:
    """
    Load all saved chat sessions for a user.
    """
    path = f"chat_sessions_{user}.json"
    if os.path.exists(path):
        try:
